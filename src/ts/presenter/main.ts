    ...
    function frame(now) {
        ...
        for (const task of tasks) {
            if (task.type === "morph") {
                ...
                (task.el as HTMLElement).style.opacity = String(
                    _ease(Math.max(0, Math.min((raw - 0.3) / 0.5, 1))) *
                        task.toOpacity,
                );
            }
        }
...
function loadSlide(then = null, transition = null) {
    ...
    const t = transition ??
        transitions[slideIndex] ?? { type: "cut", duration: 0 };
...
overview.addEventListener("click", (e) => {
    ...
    if (cell) {
        _overviewActive = +(cell as HTMLElement).dataset.index;
        _overviewCommit();
