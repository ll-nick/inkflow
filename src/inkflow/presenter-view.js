...
let slideIndex = Math.min(Math.max(0, INITIAL_POSITION.slideIndex | 0), Math.max(0, slides.length - 1));
...
function renderNext() {
  ...
  if (svg) {
    svg.querySelectorAll('[data-step]').forEach(el =>
      el.classList.toggle('active', +el.getAttribute('data-step') <= revealStep)
    );
  }
