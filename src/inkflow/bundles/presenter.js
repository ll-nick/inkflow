(() => {
  ...
  var transitions = INITIAL_TRANSITIONS;
  var slideIndex = 0;
  var step = 0;
  var _maxStepCache = null;
  var _pickerMatches = [];
  var _pickerActive = 0;
  var _overviewActive = 0;
  var _overviewCols = 1;
  var ws = null;
  ...
  function readURL() {
    ...
    if (!Number.isNaN(n) && n >= 1 && n <= slides.length) slideIndex = n - 1;
    const clicks = parseInt(
      new URLSearchParams(window.location.search).get("clicks") ?? "0",
      10
    );
    if (!Number.isNaN(clicks) && clicks >= 0) step = clicks;
  }
      ...
      for (const task of tasks) {
        if (task.type === "morph") {
          ...
          if (task.from.fill && task.toFill)
            task.el.setAttribute(
              "fill",
              _lerpColor(task.from.fill, task.toFill, e)
            );
          if (task.from.stroke && task.toStroke)
            task.el.setAttribute(
              "stroke",
              _lerpColor(task.from.stroke, task.toStroke, e)
            );
        } else if (task.type === "exit") {
          task.el.style.opacity = String(
            1 - _ease(Math.min(raw / 0.7, 1))
          );
        } else {
          task.el.style.opacity = String(
            _ease(Math.max(0, Math.min((raw - 0.3) / 0.5, 1))) * task.toOpacity
          );
        }
      }
      if (raw < 1) {
  ...
  function filterPicker(query) {
    ...
    pickerList.innerHTML = matches.map(
      (idx, pos) => `<li role="option" data-pos="${pos}" class="${pos === 0 ? "active" : ""}"><span class="pk-num">${idx + 1}</span><span class="pk-title">${slides[idx].title || ""}</span></li>`
    ).join("");
  ...
  function _pickerMoveCursor(delta) {
    ...
    _pickerActive = Math.max(
      0,
      Math.min(_pickerMatches.length - 1, _pickerActive + delta)
    );
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    if (inZone) _showFsBar();
    else _scheduleFsHide();
  });
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (overview.classList.contains("visible")) {
      ...
      if (e.key === "ArrowRight" || e.key === "l") {
        e.preventDefault();
        _overviewSetActive(_overviewActive + 1);
        return;
      }
      ...
      if (e.key === "ArrowDown" || e.key === "j") {
        e.preventDefault();
        _overviewSetActive(_overviewActive + _overviewCols);
        return;
      }
